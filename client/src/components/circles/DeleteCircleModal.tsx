...
export function DeleteCircleModal({ circle, onClose, onDeleted, onUpdated }: Props) {
  const { user } = useAuth();
  ...
  async function handleVote() {
    setLoading(true);
    try {
      const { data } = await api.post(`/circles/${circle.id}/vote-delete`);
      if (data.deleted) {
        onDeleted();
      } else {
        onUpdated(data.circle);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal title="Supprimer le Cercle" onClose={onClose}>
      <div className="space-y-5">
        {/* Warning */}
        <div className="flex gap-3 p-3 bg-red-50 border border-red-100 rounded-xl">
          <AlertTriangle size={16} className="text-red-500 flex-shrink-0 mt-0.5" />
          <p className="text-sm text-red-700">
            La suppression est <strong>irréversible</strong>. Tous les Plans et messages seront perdus.
